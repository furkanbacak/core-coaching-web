...
export async function generateMetadata({
  ...
  const idx = coachingSlugs.indexOf(slug as (typeof coachingSlugs)[number]);
  if (idx === -1) return {};
  const t = await getTranslations('coaching');
  const items = t.raw('coachingArea.items') as string[];
  return {
    title: `${items[idx]} | Core Coaching`,
    description: (t.raw('coachingArea.descriptions') as string[])[idx],
  };
}
...
export default async function CoachingDetailPage({
  ...
  const details = t.raw('coachingArea.details') as {
    intro: string;
    bullets: string[];
  }[];
